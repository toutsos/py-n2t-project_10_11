...
#private methods
    ...
    def __tokenize(self, filePath):
        ...
        # STEP 1: Convert the .jack file into a list with a line in every element
        # STEP 2: Filter the list to remove comments and whitespaces through the various filter methods in JackTokenizer
        # STEP 3: Send each line (list element) to JackTokenizer.parseLine(self,line) and this method will return a list of tagged token for each line.
        ...
        STEP 4 Send each line (list element) to JackTokenizer.parseLine(self,line) and this method will tokenize the line returning a new list with a tagged token in each element. '''
    ...
    # Instead of passing each token into this method the JackTokenizer.py returns a list with already added tags for each line it parses.
